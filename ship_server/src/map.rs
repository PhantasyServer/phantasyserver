...
use data_structs::{NewMapData, SerDeFile as _};
...
impl Map {
    pub fn new<T: AsRef<std::path::Path>>(path: T, map_obj_id: &AtomicU32) -> Result<Self, Error> {
        let data = NewMapData::load_from_mp_file(path.as_ref())?;
        let mut map = Self::new_from_data(data, map_obj_id)?;
        map.load_path = path.as_ref().to_owned();
        Ok(map)
    }
    pub fn new_from_data(data: NewMapData, map_obj_id: &AtomicU32) -> Result<Self, Error> {
    ...
    pub async fn reload_objs(&mut self) -> Result<(), Error> {
        ...
        let map = NewMapData::load_from_mp_file(&self.load_path)?;
    ...
    pub async fn move_player(&mut self, id: PlayerId, mapid: MapId) -> Result<(), Error> {
        let Some(player) = self.remove_player(id).await else {
            return Err(Error::InvalidInput("move_player"));
        };
    ...
    fn load_objects(
            ...
            .cloned()
        {
            if user.packet_type == PacketType::Vita {
                if let Some(lua_code) = map_data.luas.get(obj.data.name.as_str()) {
    ...
    pub async fn interaction(
        ...
        let Some((_, mapid, _)) = self.players.iter().find(|p| p.0 == sender_id) else {
            return Err(Error::InvalidInput("interaction"));
        };
        let mapid = *mapid;
    ...
    async fn run_lua<S: serde::Serialize>(
        &mut self,
        sender_id: PlayerId,
        mapid: MapId,
        packet: &S,
        ...
        let mut to_send = vec![];
        {
            ...
            lua.scope(|scope| {
                ...
                let move_player =
                    scope.create_function_mut(|_, (receiver, mapid): (u32, u32)| {
...
where
    F: FnMut(PlayerId, MapId, MutexGuard<User>),
{
